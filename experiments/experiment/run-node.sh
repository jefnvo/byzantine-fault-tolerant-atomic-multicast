...
if [ "${G_ID:0:1}" = "g" ]; then 
    ...
    java -cp '../../target/*' ch.usi.inf.dslab.bftamcast.server.BatchServerGlobal -i $N_ID -g ${G_ID:1:1} -gc $GLOBALGROUPS -lcs $LOCALGROUPS
else
    echo "LOCAL SERVER"
    java -cp '../../target/*' ch.usi.inf.dslab.bftamcast.server.Server -i $N_ID -g $G_ID -lc group-$G_ID
fi
